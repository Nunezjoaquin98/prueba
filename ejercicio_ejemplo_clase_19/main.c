 ...
 * \return void
 */
void loadData(sPersona* p)
{
    while(!getStringLetras("Ingrese el nombre: ",p->name))
    {
        printf("\nError, debe ingresar solo letras.\n");
        system("pause");
    }

    while(!getStringLetras("Ingrese el apellido: ",p->surname))
    {
        printf("\nError, debe ingresar solo letras.\n");
        system("pause");
    }

    while(!getStringNumeros("Ingrese la edad: ",p->age))
    {
 ...
 * \return 0: error, 1:OK
 */
int savePerson(sPersona* p)
{
    FILE* fp;
    fp=fopen("bin.dat","a+b"); // append binario, lo crea si no existe
    if(fp==NULL)
    {
        printf("Error opening file");
        return 0;
    }
    //fwrite(p,sizeof(struct S_Person),1,fp);
    fwrite(p,sizeof(sPersona),1,fp); // escribo una struct al final
